...
import { Agent, ConnectionRecord, PresentationPreview, PresentationPreviewAttribute } from '@aries-framework/core'
...
export const send_proof_proposal = async (annelein: Agent, connectionRecord: ConnectionRecord) => {
  annelein.events.on(ProofEventTypes.ProofStateChanged, async ({ payload }: ProofStateChangedEvent) => {
