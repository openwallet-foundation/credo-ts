...
2. Add `@aries-framework/core`, `@aries-framework/react-native`, `buffer`, `react-native-fs`, and `react-native-get-random-values` to your project.

```bash
yarn add @aries-framework/core @aries-framework/react-native buffer react-native-fs react-native-get-random-values
```
