...
export class IndyVdrSovDidRegistrar implements DidRegistrar {
  ...
  public async create(agentContext: AgentContext, options: IndyVdrSovDidCreateOptions): Promise<DidCreateResult> {
    ...
    try {
      ...
      const pool = indyNamespace
        ? this.indyVdrPoolService.getPoolForNamespace(indyNamespace)
        : this.indyVdrPoolService.pools[0]
  ...
  public async registerPublicDid(
    ...
    try {
      ...
      const key = await agentContext.wallet.createKey({ keyType: KeyType.Ed25519, seed: 'bla' })
...
export interface IndyVdrSovDidCreateOptions extends DidCreateOptions {
  method: 'sov'
  did?: undefined
