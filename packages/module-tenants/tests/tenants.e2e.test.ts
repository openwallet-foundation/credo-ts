...
describe('Tenants E2E', () => {
  ...
  test('create get and delete a tenant', async () => {
    // Create tenant
    let tenantRecord1 = await agent1.tenants.createTenant({
      config: {
        label: 'Tenant 1',
      },
    })
    ...
    tenantRecord1 = await agent1.tenants.getTenantById(tenantRecord1.id)

    // Get tenant agent
    const tenantAgent = await agent1.tenants.getTenantAgent({
      tenantId: tenantRecord1.id,
    })
    await tenantAgent.endSession()

    // Delete tenant agent
    await agent1.tenants.deleteTenantById(tenantRecord1.id)

    // Can not get tenant agent again
    await expect(agent1.tenants.getTenantAgent({ tenantId: tenantRecord1.id })).rejects.toThrow(
  ...
  test('create a connection between two tenants within the same agent', async () => {
    // Create tenants
    const tenantRecord1 = await agent1.tenants.createTenant({
      config: {
        label: 'Tenant 1',
      },
    })
    const tenantRecord2 = await agent1.tenants.createTenant({
      config: {
        label: 'Tenant 2',
      },
    })

    const tenantAgent1 = await agent1.tenants.getTenantAgent({
      tenantId: tenantRecord1.id,
    })

    const tenantAgent2 = await agent1.tenants.getTenantAgent({
    ...
    await agent1.tenants.deleteTenantById(tenantAgent1.context.contextCorrelationId)
    await agent1.tenants.deleteTenantById(tenantAgent2.context.contextCorrelationId)
  })
  ...
  test('create a connection between two tenants within different agents', async () => {
    ...
    const tenantRecord2 = await agent2.tenants.createTenant({
      config: {
        label: 'Agent 2 Tenant 1',
      },
    })
    const tenantAgent2 = await agent2.tenants.getTenantAgent({
      tenantId: tenantRecord2.id,
    })
    ...
    // Delete tenants (will also delete wallets)
    await agent1.tenants.deleteTenantById(tenantRecord1.id)
    await agent2.tenants.deleteTenantById(tenantRecord2.id)
  })

  test('perform actions within the callback of withTenantAgent', async () => {
    const tenantRecord = await agent1.tenants.createTenant({
      config: {
        label: 'Agent 1 Tenant 1',
      },
    })

    await agent1.tenants.withTenantAgent({ tenantId: tenantRecord.id }, async (tenantAgent) => {
    ...
    await agent1.tenants.deleteTenantById(tenantRecord.id)
  })
})
