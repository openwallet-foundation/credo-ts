...
const agentConfig = getAgentConfig('AnonCreds format services using anoncreds-rs', { logger: new ConsoleLogger(LogLevel.debug)})
...
describeRunInNodeVersion([18], 'AnonCreds format services using anoncreds-rs', () => {
  ...
  test.only('issuance and verification flow starting from proposal without negotiation and with revocation', async () => {
    const schema = await anonCredsIssuerService.createSchema(agentContext, {
      attrNames: ['name', 'age'],
      issuerId: indyDid,
      name: 'Employee Credential',
    ...
    if (
      ...
      !revocationStatusListState.timestamp
    ) {
