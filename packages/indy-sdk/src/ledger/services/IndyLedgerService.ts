import type { AnonCredsRegistry, RegisterSchemaOptions, RegisterSchemaReturn } from '../../../../anoncreds/src'
import type { IndyPoolConfig } from '../IndyPool'
import type { AgentContext } from '@aries-framework/core'
import type { CredDef, default as Indy, NymRole, Schema } from 'indy-sdk'

import { AgentDependencies, InjectionSymbols, IndySdkError, Logger, injectable, inject } from '@aries-framework/core'

import { isIndyError } from '../../error'
import { IndySdkIssuerService } from '../../services/IndySdkIssuerService'
import { didFromCredentialDefinitionId, didFromRevocationRegistryDefinitionId, didFromSchemaId } from '../../util/did'

import { IndyPoolService } from './IndyPoolService'

@injectable()
export class IndyLedgerService implements AnonCredsRegistry {
  private indy: typeof Indy
  private logger: Logger

  private indyIssuer: IndySdkIssuerService
  private indyPoolService: IndyPoolService

  public constructor(
    @inject(InjectionSymbols.AgentDependencies) agentDependencies: AgentDependencies,
    @inject(InjectionSymbols.Logger) logger: Logger,
    indyIssuer: IndySdkIssuerService,
  ...
  public async registerSchema(options: RegisterSchemaOptions): Promise<RegisterSchemaReturn> {
    const { agentContext, issuerId, schema: schemaTemplate } = options

    const pool = this.indyPoolService.getPoolForNamespace()

    try {
      this.logger.debug(`Register schema on ledger '${pool.id}' with id '${issuerId}'`, schemaTemplate)
      const { name, attrNames, version } = schemaTemplate
      const schema = await this.indyIssuer.createSchema(agentContext, {
        originDid: issuerId,
        name,
        version,
        attrNames,
      ...
      const request = await this.indy.buildSchemaRequest(issuerId, schema)

      const response = await this.indyPoolService.submitWriteRequest(agentContext, pool, request, issuerId)
  ...
  public async registerCredentialDefinition(
    ...
    try {
      ...
      const credentialDefinition = await this.indyIssuer.createCredentialDefinition(agentContext, {
        issuerId: did,
        schema: { id: schema.id, value: schema },
        tag,
        signatureType,
        supportRevocation,
      })
