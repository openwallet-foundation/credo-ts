...
export function getKeysFromFormatData(formatData: GetFormatDataReturn<[IndyProofFormat]>): {
  proposeKey1: string
  proposeKey2: string
  requestKey1: string
  requestKey2: string
} {
  const proofRequest: IndyProofRequest | undefined = formatData.request?.indy ?? undefined
  const proofProposal: IndyProofRequest | undefined = formatData.proposal?.indy ?? undefined
  let proposeKey1, proposeKey2: string
  ...
  const proposeAttributes: Map<string, ProofAttributeInfo> = proofProposal[
    'requested_attributes' as keyof IndyProofRequest
  ] as Map<string, ProofAttributeInfo>

  // these keys are dynamically generated from UUID

  // eslint-disable-next-line prefer-const
  proposeKey1 = Object.keys(proposeAttributes)[1]

  const proposePredicates: Map<string, ProofAttributeInfo> = proofProposal[
    'requested_predicates' as keyof IndyProofRequest
  ] as Map<string, ProofAttributeInfo>

  // eslint-disable-next-line prefer-const
  proposeKey2 = Object.keys(proposePredicates)[0]

  // eslint-disable-next-line prefer-const
  let requestKey1, requestKey2: string
  ...
  // eslint-disable-next-line prefer-const
  requestKey1 = Object.keys(attributes)[1]
  ...
  // eslint-disable-next-line prefer-const
  requestKey2 = Object.keys(predicates)[0]
