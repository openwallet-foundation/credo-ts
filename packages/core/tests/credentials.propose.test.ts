...
describe('credentials', () => {
  ...
  test('Alice starts with V2 (Indy format) credential proposal to Faber', async () => {
    ...
    testLogger.test('Faber sends credential offer to Alice')
    await faberAgent.credentials.acceptCredentialProposal(faberCredentialRecord.id, {
      comment: 'some comment about credential',
    })
    // faberCredentialRecord = await faberAgent.credentials.acceptCredentialProposal(faberCredentialRecord.id, {
    //   comment: 'some comment about credential',
    // })
