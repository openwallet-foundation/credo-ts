...
export function createKmsKeyPairClass(agentContext: AgentContext) {
  return class KmsKeyPair extends LdKeyPair {
     ...
     */
    public signer(): { sign: (data: { data: Uint8ArrayBuffer | Uint8ArrayBuffer[] }) => Promise<Uint8ArrayBuffer> } {
      // wrap function for conversion
      const wrappedSign = async (data: { data: Uint8ArrayBuffer | Uint8ArrayBuffer[] }): Promise<Uint8ArrayBuffer> => {
     ...
     */
    public verifier(): {
      verify: (data: { data: Uint8ArrayBuffer | Uint8ArrayBuffer[]; signature: Uint8ArrayBuffer }) => Promise<boolean>
    } {
      const wrappedVerify = async (data: {
        data: Uint8ArrayBuffer | Uint8ArrayBuffer[]
