...
export async function extractDidDocument(agent: Agent, connectionRecord: ConnectionRecord) {
  ...
  if (oldDidDocJson) {
    const oldDidDoc = JsonTransformer.fromJSON(oldDidDocJson, DidDoc, { validate: true })
  ...
  if (oldTheirDidDocJson) {
    const oldTheirDidDoc = JsonTransformer.fromJSON(oldTheirDidDocJson, DidDoc, { validate: true })
...
export async function migrateToOobRecord(
  ...
  if (oldInvitationJson) {
    const oldInvitation = JsonTransformer.fromJSON(oldInvitationJson, ConnectionInvitationMessage, { validate: true })
