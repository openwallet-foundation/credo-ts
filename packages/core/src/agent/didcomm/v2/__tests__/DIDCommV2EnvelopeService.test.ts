...
describe('DIDCommV2EnvelopeService', () => {
  ...
  mockFunction(didResolverServiceProvider.resolve).mockImplementation(async (agentContext, didUrl, _options) => {
    switch (didUrl) {
  ...
  const didResolverService = new DIDResolverService(agentContextProvider, didResolverServiceProvider)
  ...
  test("packMessageEncrypted uses recipient's routing", async () => {
    ...
    const unpackedByMediator1 = await mediator1EnvelopeService.unpackMessage(agentContext, packedForMediator1)
    ...
    const unpackedByMediator2 = await mediator2EnvelopeService.unpackMessage(agentContext, packedForMediator2)
    ...
    const unpackedByBob = await bobEnvelopeService.unpackMessage(agentContext, packedForBob)
