...
describe('X509ServiceModule', () => {
  test('registers dependencies on the dependency manager', async () => {
    ...
    const x509ServiceModule = new X509Module(options)

    x509ServiceModule.register(dependencyManager)
    ...
    expect(x509ServiceModule.config.trustedCertificates).toBeDefined()
    expect(x509ServiceModule.config.trustedCertificates).toHaveLength(1)
    expect(x509ServiceModule.config.trustedCertificates).toContain('certificate')

    x509ServiceModule.config.addTrustedCertificate('certificate2')
    expect(x509ServiceModule.config.trustedCertificates).toHaveLength(2)

    x509ServiceModule.config.setTrustedCertificates(undefined)
    expect(x509ServiceModule.config.trustedCertificates).toBeUndefined()
  })
})
