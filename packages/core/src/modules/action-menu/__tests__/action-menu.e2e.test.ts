...
import { ActionMenuRecord } from '../repository'

import { waitForActionMenuRecord } from './helpers'

const faberAgentOptions = getAgentOptions('Faber Action Menu', {
  endpoints: ['rxjs:faber'],
})

const aliceAgentOptions = getAgentOptions('Alice Action Menu', {
...
describe('Action Menu', () => {
  ...
  test('Menu navigation', async () => {
    ...
    faberActionMenuRecord = await faberAgent.actionMenu.sendMenu({
      connectionId: faberConnection.id,
      menu: submenu1,
    })
