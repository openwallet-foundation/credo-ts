...
export class W3cV2JwtCredentialService {
  ...
  public async signCredential(
    ...
    const jwtPayload = new JwtPayload(JsonTransformer.toJSON(options.credential))
  ...
  public async verifyCredential(
    ...
    try {
      ...
      if (verifyCredentialStatus && !credential.resolvedCredential.status) {
        validationResults.validations.credentialStatus = {
          isValid: true,
        }
      } else if (verifyCredentialStatus && credential.resolvedCredential.status) {
        validationResults.validations.credentialStatus = {
          isValid: false,
  ...
  public async signPresentation(
    ...
    const jwtPayload = new JwtPayload(JsonTransformer.toJSON(options.presentation))
