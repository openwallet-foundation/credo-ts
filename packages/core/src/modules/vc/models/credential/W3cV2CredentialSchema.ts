...
const jsonToClass = (v: unknown) => {
  if (!v || typeof v !== 'object') {
    throw new CredoError('Invalid plain W3cV2CredentialSchema')
  }

  if (isInstance(v, W3cV2CredentialSchema)) {
    return v
  }

  return new W3cV2CredentialSchema(v as W3cV2CredentialSchemaOptions)
}

const classToJson = (v: unknown) => {
  if (v instanceof W3cV2CredentialSchema) {
...
export function W3cV2CredentialSchemaTransformer() {
  return Transform(({ value, type }: { value: W3cV2CredentialSchemaOptions; type: TransformationType }) => {
    if (type === TransformationType.PLAIN_TO_CLASS) {
      if (Array.isArray(value) && value.length === 0) {
        throw new CredoError('At least one credential schema is required')
      }
