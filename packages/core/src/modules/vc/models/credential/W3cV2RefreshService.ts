...
const jsonToClass = (v: unknown) => {
  if (!v || typeof v !== 'object') {
    throw new CredoError('Invalid plain W3cV2CredentialRefreshService')
  }

  if (isInstance(v, W3cV2RefreshService)) {
    return v
  }

  return new W3cV2RefreshService(v as W3cV2RefreshServiceOptions)
}

const classToJson = (v: unknown) => {
  if (v instanceof W3cV2RefreshService) {
...
export function W3cV2RefreshServiceTransformer() {
  return Transform(({ value, type }: { value: W3cV2RefreshServiceOptions; type: TransformationType }) => {
    if (type === TransformationType.PLAIN_TO_CLASS) {
      if (Array.isArray(value) && value.length === 0) {
        throw new CredoError('At least one refresh service is required')
      }
