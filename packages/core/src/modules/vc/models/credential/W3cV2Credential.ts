...
export class W3cV2Credential {
  public constructor(options: W3cV2CredentialOptions) {
    if (options) {
      ...
      if (options.name) {
        this.name =
          typeof options.name === 'string'
            ? options.name
            : mapSingleOrArray(options.name, (n) => (n instanceof W3cV2LocalizedValue ? n : new W3cV2LocalizedValue(n)))
      }
      ...
      if (options.description) {
        this.description =
          typeof options.description === 'string'
            ? options.description
            : mapSingleOrArray(options.description, (n) =>
                n instanceof W3cV2LocalizedValue ? n : new W3cV2LocalizedValue(n)
              )
      }

      this.issuer =
        typeof options.issuer === 'string' || options.issuer instanceof W3cV2Issuer
          ? options.issuer
          : new W3cV2Issuer(options.issuer)
      ...
      if (options.status) {
        this.status = mapSingleOrArray(options.status, (status) =>
      ...
      if (options.refreshService) {
        this.refreshService = mapSingleOrArray(options.refreshService, (service) =>
          service instanceof W3cV2RefreshService ? service : new W3cV2RefreshService(service)
        )
      }

      if (options.termsOfUse) {
        this.termsOfUse = mapSingleOrArray(options.termsOfUse, (tos) =>
          tos instanceof W3cV2TermsOfUse ? tos : new W3cV2TermsOfUse(tos)
        )
      }

      if (options.evidence) {
        this.evidence = mapSingleOrArray(options.evidence, (evidence) =>
