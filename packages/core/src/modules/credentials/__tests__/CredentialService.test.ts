...
import { Dispatcher } from '@aries-framework/core'
import { MediationRecipientService } from '../../routing'
import { CredentialProtocolVersion } from '../CredentialProtocolVersion'
import { OfferCredentialOptions } from '../interfaces'
...
const init = () => {
  ...
  const agent: Agent = new Agent(config, dependencies)
  didCommMessageRepository = agent.injectionContainer.resolve(DidCommMessageRepository)
  const dispatcher = agent.injectionContainer.resolve(Dispatcher)
  ...
  const legacyCredentialService = new V1LegacyCredentialService(
...
describe('CredentialService', () => {
  ...
  describe('createCredentialOffer', () => {
    ...
    test(`creates credential record in ${CredentialState.OfferSent} state with offer, thread ID`, async () => {
      ...
      // pull out the offer message from the didcomm message repository
      // const offerMessage = await didCommMessageRepository.getAgentMessage({
      //   associatedRecordId: createdCredentialRecord.id,
