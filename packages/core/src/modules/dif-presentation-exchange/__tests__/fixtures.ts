...
import { MdocRecord, MdocRepository } from '../../mdoc'
...
export const createDifPresentationExchangeTestContext = () => {
  ...
  return {
    ...
    sdJwtVcRepository: agentContext.dependencyManager.resolve(SdJwtVcRepository),
    mdocRepository: agentContext.dependencyManager.resolve(MdocRepository),
    pexService: new DifPresentationExchangeService(agentContext.dependencyManager.resolve(W3cCredentialService)),
  }
}

export const createSdJwtVcRecord = () =>
  new SdJwtVcRecord({
    credentialInstances: [
      {
        compactSdJwtVc:
...
export const createMdocRecord = () =>
  new MdocRecord({
    credentialInstances: [{ issuerSignedBase64Url: sprindFunkeTestVectorBase64Url }],
  })

export const createRandomMdocRecord = () =>
  new MdocRecord({
    credentialInstances: [
      {
        issuerSignedBase64Url:
...
export const presentationDefinition: DifPresentationExchangeDefinitionV2 = {
  ...
  input_descriptors: [
    {
      ...
      constraints: {
        ...
        fields: [
          ...
          { path: ['$.vct'], filter: { type: 'string', enum: ['https://example.eudi.ec.europa.eu/mdl/1'] } },
        ],
      },
    },
    {
    ...
    },
    {
      ...
      constraints: {
        limit_disclosure: 'required',
        fields: [{ intent_to_retain: false, path: ["$['eu.europa.ec.eudi.pid.1']['birth_date']"] }],
      },
    },
    {
      ...
      constraints: {
        limit_disclosure: 'required',
        fields: [{ intent_to_retain: false, path: ["$['hello']['world']"] }],
      },
    },
  ],
}
