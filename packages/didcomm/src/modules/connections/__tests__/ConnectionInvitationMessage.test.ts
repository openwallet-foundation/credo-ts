...
describe('DidCommConnectionInvitationMessage', () => {
  ...
  describe('toUrl', () => {
    ...
    it('should use did:sov:BzCbsNYhMrjHiqZDTUASHg;spec/connections/1.0/invitation as type if useDidSovPrefixWhereAllowed is set to true', async () => {
      ...
      expect(JsonEncoder.fromBase64Url(encodedInvitation)['@type']).toBe(
  ...
  describe('fromUrl', () => {
    ...
    it('should correctly convert a valid invitation url to a `DidCommConnectionInvitationMessage` with `c_i` as parameter', async () => {
      const invitationUrl =
