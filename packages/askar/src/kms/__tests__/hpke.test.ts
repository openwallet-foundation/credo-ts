...
const agentContext = getAgentContext({
  ...
  agentConfig: getAgentConfig('AskarKeyManagementServiceHpke'),
  registerInstances: [
...
describe('AskarKeyManagementService HPKE', () => {
  ...
  it('decrypts the RFC 9180 A.3 test vector with a key held by askar', async () => {
  ...
  it.each(['HPKE-0', 'HPKE-7'] as const)('round-trips %s with a P-256 key created in askar', async (algorithm) => {
    ...
    expect(encapsulatedKey?.[0]).toBe(0x04)
  ...
  it('round-trips HPKE-3 with an X25519 key created in askar', async () => {
    const { keyId, publicJwk } = await service.createKey(agentContext, {
  ...
  it('fails to decrypt when info does not match', async () => {
    ...
    ).rejects.toThrow('Error decrypting with key')
  })
  ...
  it('fails to decrypt when the ciphertext is tampered with', async () => {
    ...
    await expect(
      service.decrypt(agentContext, {
        key: {
          keyAgreement: { algorithm: 'HPKE-0', keyId, encapsulatedKey: encapsulatedKey as Uint8Array },
        },
        decryption: { algorithm: 'HPKE' },
        encrypted: tampered,
      })
  ...
  it('decrypts the RFC 9180 A.1 test vector with a key held by askar', async () => {
    const { keyId } = await service.importKey(agentContext, {
  ...
  it('uses a fresh ephemeral key for every seal', async () => {
    ...
    const encryptOptions = {
      key: { keyAgreement: { algorithm: 'HPKE-0', externalPublicJwk: publicJwk } },
      encryption: { algorithm: 'HPKE' },
      data: TypedArrayEncoder.fromUtf8String('the quick brown fox'),
    } as const
  ...
  it('rejects a key whose `alg` restricts it to another HPKE suite', async () => {
    ...
    await expect(
      service.encrypt(agentContext, {
        key: { keyAgreement: { algorithm: 'HPKE-7', externalPublicJwk: { ...publicJwk, alg: 'HPKE-0' } } },
        encryption: { algorithm: 'HPKE' },
        data: TypedArrayEncoder.fromUtf8String('secret'),
      })
  ...
  it('rejects a key whose usage does not allow key derivation', async () => {
    ...
    await expect(
      service.encrypt(agentContext, {
        key: { keyAgreement: { algorithm: 'HPKE-0', externalPublicJwk: { ...publicJwk, use: 'sig' } } },
        encryption: { algorithm: 'HPKE' },
        data: TypedArrayEncoder.fromUtf8String('secret'),
      })
...
/**
 * The askar backend seals with the plain HPKE suite but opens through a KEM that delegates the
 * Diffie-Hellman step to askar, so the recipient private key never leaves the store. These check
 * that custom `Decap` against the stock implementation the node backend uses, in both directions.
 */
describe('AskarKeyManagementService HPKE interop with NodeKeyManagementService', () => {
  const nodeService = new NodeKeyManagementService(new NodeInMemoryKeyManagementStorage())

  it.each(['HPKE-0', 'HPKE-7'] as const)('opens a %s message sealed by the node backend', async (algorithm) => {
    const { keyId, publicJwk } = await service.createKey(agentContext, {
      type: { kty: 'EC', crv: 'P-256' },
      keyId: `hpke-interop-node-seals-${algorithm}`,
    })
    ...
    const { encrypted, encapsulatedKey } = await nodeService.encrypt(agentContext, {
      key: { keyAgreement: { algorithm, externalPublicJwk: publicJwk, info } },
      encryption: { algorithm: 'HPKE', aad },
      data,
    })
  ...
  it('opens an HPKE-3 message sealed by the node backend', async () => {
    const { keyId, publicJwk } = await service.createKey(agentContext, {
      type: { kty: 'OKP', crv: 'X25519' },
      keyId: 'hpke-interop-node-seals-x25519',
    })
  ...
  it('seals a message the node backend can open', async () => {
    const { keyId, publicJwk } = await nodeService.createKey(agentContext, {
      type: { kty: 'EC', crv: 'P-256' },
      keyId: 'hpke-interop-askar-seals',
    })
    ...
    const decrypted = await nodeService.decrypt(agentContext, {
      key: { keyAgreement: { algorithm: 'HPKE-0', keyId, encapsulatedKey: encapsulatedKey as Uint8Array, info } },
      decryption: { algorithm: 'HPKE' },
      encrypted,
    })
